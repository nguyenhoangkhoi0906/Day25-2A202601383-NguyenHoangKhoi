    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open":
                ...
                recovery_times_ms.append((float(entry["ts"]) - opened_ts) * 1000)
                opened_ts = None
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        # A provider forced to near-total failure should be rescued by fallback;
        # otherwise we expect the gateway to keep normal availability high.
        if any(rate >= 0.99 for rate in scenario.provider_overrides.values()):
